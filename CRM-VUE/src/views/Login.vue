      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$store
            ...
            .then(() => {
              if (
                ...
                localStorage.setItem(
                  "id",
                  this.$store.state.Login.loginInfo.data[0].id
                );
                localStorage.setItem(
                  "trueName",
                  this.$store.state.Login.loginInfo.data[0].trueName
                );
                localStorage.setItem(
                  "userName",
                  this.$store.state.Login.loginInfo.data[0].userName
                );
                // this.$router.push("/home");
              } else {
                this.$message.error(
                  this.$store.state.Login.loginInfo.data[0].loginTip
                );
          ...
          this.$store
            ...
            .catch((e) => {
              this.$message.error("获取用户角色时发生错误：" + e);
              // this.loading = false
            });
        } else {
          return false;
